...
def generate_cache(json_path, img_path):
    ...
    print("原coco格式的json文件中的annotation的个数为", len(annotations['annotations']))
    ...
    for img_id, anns in tqdm(img_to_anns.items()):
        ...
        for ann in anns:
            ...
            #利用ultralytics工具函数，将box转换为归一化的xywh格式
            
            if box[2] <= 0 or box[3] <= 0:
                continue

            #这里与yoloe不同，yoloe做了.lower().strip()处理，但是yolo-world由于原来没有做这个处理，所以生成缓存的时候也不做这个处理
