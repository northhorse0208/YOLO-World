...
import os
import json
from generate_label_embedding import generate_yolo_world_embedding
import torch
...
if __name__ == '__main__':
    ...
    flickr_cache_path = Path('./../../../dataset/flickr 30k/final_flickr_separateGT_train_segm.cache')
    obtain_cat_freq(flickr_cache_path, cat_name_freq)

    mixed_grounding_cache_path = Path('./../../../dataset/GQA/final_mixed_train_no_coco_segm.cache')
    ...
    with open('embedding_cache/global_grounding_neg_cat.json', 'w') as f:
        json.dump(global_neg_cat, f, indent=2)

    # 下面是yoloe 用mobileclip:blt生成负样本的embedding
    ...
    #下面是使用openai clip生成yoloworld所需的负样本embedding
    model = 'openai/clip-vit-base-patch32'
    global_neg_embeddings = generate_yolo_world_embedding(model, global_neg_cat)
    os.makedirs(f'embedding_cache/{model}', exist_ok=True)
    torch.save(global_neg_embeddings, f'embedding_cache/{model}/global_grounding_neg_embeddings.pt')
